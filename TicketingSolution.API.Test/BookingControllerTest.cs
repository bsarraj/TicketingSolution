...
namespace TicketingSolution.API.Test
{
    public class BookingControllerTest
    {
        ...
        [Theory]
        [InlineData(1, true, typeof(OkObjectResult), BoockingResultFlag.Success)]
        [InlineData(0, false, typeof(BadRequestObjectResult), BoockingResultFlag.Failure)]
        public async Task Should_Call_Booking_Method_When_Valid(int expectedMethodCalls, bool isModelValid, Type expectedActionResultType, BoockingResultFlag bookingResultFlag)
        {
            // Arrange
            if (!isModelValid)
            {
            ...
            _request.Flag = bookingResultFlag;
